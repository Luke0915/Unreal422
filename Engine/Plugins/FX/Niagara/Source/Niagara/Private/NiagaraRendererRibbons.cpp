	...
	if (DynamicIndexAllocation.IsValid())
	{
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			if (VisibilityMap & (1 << ViewIndex))
			{
				...
				bool bInvertOrder = ((StartDist > EndDist) && DrawDirection == ENiagaraRibbonDrawDirection::BackToFront)
				...
				PerViewUniformParameters.UseCustomFacing = FacingMode == ENiagaraRibbonFacingMode::Custom ? 1 : 0;
				...
				PerViewUniformParameters.UV0TilingDistance = UV0TilingDistance;
				PerViewUniformParameters.UV1TilingDistance = UV1TilingDistance;
